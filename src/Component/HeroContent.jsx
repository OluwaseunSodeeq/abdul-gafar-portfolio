...
function HeroContent() {
  return (
    <div className="font-Helvetica flex flex-col items-center gap-4 py-3 lg:py-0 px-4 lg:gap-6">
      {cardsContent.map((each) => (
        <div
          key={each.heading}
          className="py-4 px-6 rounded-lg bg-second-logo-text shadow-2xl lg:w-[886px] lg:h-auto lg:text-left"
        >
          <div className="w-36 h-11 flex justify-start gap-2 items-center lg:items-start lg:w-44 lg:h-16 lg:flex-col ">
            <div>{each.icon}</div>
            <h3 className=" font-normal text-xl  leading-9 lg:leading-7 text-user-color">
              {each.heading}
            </h3>
          </div>
          <p className="font-bold text-sm md:text-base leading-9 lg:leading-10 text-content-color">
            {each.text}
          </p>
        </div>
      ))}
